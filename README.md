...
Para atender aos requisitos de negócios complexos de um hotel, o banco de dados foi normalizado seguindo as regras formais e conta com:
...
4. **Controle de Transações e Propriedades ACID:** Implementação de transações estruturadas (`BEGIN`, `COMMIT`, `ROLLBACK`) para gerenciar fluxos críticos e concorrentes do hotel (como check-in expresso, check-out financeiro e tratamento seguro de erros operacionais ou desistências no balcão).
...
* **Filtros de Agrupamento:** Uso de `GROUP BY` e `HAVING` aplicados à logística operacional (como identificação de picos de ocupação por andar do hotel).
...
* **`/3-transacoes`:** Scripts contendo os cenários práticos de transações SQL simulando regras de negócio em ambiente controlado com Commit e Rollback.
...
* **`/4-arquivos e imagens do ML`:** Prints das consultas (SELECT) e das transações ACID executadas com sucesso no pgAdmin, comprovando o funcionamento prático e a integridade do banco.